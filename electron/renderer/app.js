...
generate3dBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    const useFeatureMerge = !!(
      baseSprite &&
      lastTwoDResult.featureCropUrlFor3D &&
      lastTwoDResult.featureBbox
    );
    if (useFeatureMerge) {
      logLine(`merging into ${baseSprite}: original mesh preserved, only addition is generated by Meshy`);
    }
    const started = await provider.startGeneration({
      imageUrl: useFeatureMerge ? lastTwoDResult.featureCropUrlFor3D : lastTwoDResult.imageUrlFor3D,
      prompt: lastTwoDResult.prompt || undefined,
      mode: 'object',
      ...(useFeatureMerge ? {
        baseSprite,
        featureBbox: lastTwoDResult.featureBbox,
      } : {}),
    });
